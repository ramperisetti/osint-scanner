...
DEMO_GITHUB_DATA = {
    "github":     {"org": "github",     "repos": 371},
    "microsoft":  {"org": "microsoft",  "repos": 4800},
    "google":     {"org": "google",     "repos": 2400},
    "meta":       {"org": "facebook",   "repos": 300},
    "netflix":    {"org": "netflix",    "repos": 250},
    "airbnb":     {"org": "airbnb",     "repos": 120},
    "stripe":     {"org": "stripe",     "repos": 95},
    "shopify":    {"org": "shopify",    "repos": 180},
    "uber":       {"org": "uber-go",    "repos": 130},
    "linkedin":   {"org": "linkedin",   "repos": 90},
    "dropbox":    {"org": "dropbox",    "repos": 80},
    "adobe":      {"org": "adobe",      "repos": 420},
    "twitter":    {"org": "twitter",    "repos": 210},
    "coinbase":   {"org": "coinbase",   "repos": 175},
    "spotify":    {"org": "spotify",    "repos": 200},
}
...
async def scan_github(domain: str) -> Tuple[list[dict], Union[int, str]]:
    ...
    # Try live scan first if token is available
    if GITHUB_TOKEN:
        result = await _live_scan(domain, company)
        if result[0]:  # live scan returned findings
            return result
...
async def _live_scan(domain: str, company: str) -> Tuple[list[dict], Union[int, str]]:
    print(f"[GitHub] Starting live scan for {company}, token: {bool(GITHUB_TOKEN)}")
    findings = []
    ...
    try:
        async with httpx.AsyncClient(timeout=8.0, headers=_headers()) as client:
            ...
            if r.status_code == 404:
                ...
                print(f"[GitHub] Search status: {r2.status_code}")
                if r2.status_code == 200 and r2.json().get("items"):
                    login = r2.json()["items"][0]["login"]
                    print(f"[GitHub] Found org: {login}")
    ...
    if not findings:
        print(f"[GitHub] No findings for {company}")
        return [], NO_DATA
 
    print(f"[GitHub] Live scan: {len(findings)} findings for {company}")
    return findings, _score(findings)
